...
@router.post("/upload")
async def upload_file(file: UploadFile = File(...), db: Session = Depends(get_db), background_tasks: BackgroundTasks = None):
    try:
        ...
        background_tasks.add_task(upload_to_s3, file_content, file.filename)    # Upload the file to S3 in the background
